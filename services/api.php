...
//fetch salt
  ...
  $this->response('', 204);	// If no records "No Content" status
    ...
    private function getFunctionalities(){
      ...
      $query="SELECT functionalityId, description, framework, guideline FROM functionality WHERE requirementId = '$id' order by functionalityId asc";
    ...
    private function getExamples(){
      ...
      $query="SELECT exampleId, title, description, targetGroup, screenshot FROM example WHERE functionalityId = '$id' order by exampleId asc";
...
$user = $this->changePassword($user);
			...
			$column_names = array('userType', 'email', 'password', 'salt', 'firstName', 'familyName', 'jobDesc', 'organisationName', 'organisationAddress', 'organisationPostalCode', 'organisationEmail', 'organisationPhone');
