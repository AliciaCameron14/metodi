...
app.controller('mainController', ['$scope', '$rootScope', '$route', 'services',
  ...
  'FileUploader', '$sce',
  function($scope, $rootScope, $route, services, $location, steps, chain,
    requirements, $uibModal, FileUploader, $sce) {

    $scope.chain = {};
    $scope.functionalityTip =
      "Zie hieronder een lijst met functionaliteiten. Een functionaliteiten geeft een basisprincipe met concrete voorbeelden/implementaties weer, die je kan gebruiken om aan de bovenliggende requirement tegemoet te komen.";
    $scope.basisTip =
      "Ieder voorbeeld is voor een specifieke doelgroep bedoeld. We onderscheiden de volgende doelgroepen: Ouder, Leerkracht en kind. Kies 'weergeven' om de voorbeelden te tonen.";
    ...
    $scope.selectExample = function(example) {
      ...
      $scope.chain.example.path = example.requirementId + example.functionalityId +
        example.exampleId;
    ...
    $scope.viewImage = function(image) {
      $scope.image = image;
      $scope.imagePath = $scope.chain.example.requirementId + $scope.chain
        .example.functionalityId + $scope.chain.example.exampleId;
    ...
    $scope.deleteFunctionality = function(item) {
      ...
      return ($uibModal.open({
        ...
        services.deleteFunctionality($scope.item).then(function(
          data) {
          if (data) {
            $scope.allFunctionalities = data.data;
          }
    ...
    $scope.addExampleImg = function(item) {
      ...
      $scope.item = angular.copy(item);
      $scope.item.folder = $scope.item.requirementId + $scope.item.functionalityId +
        $scope.item.exampleId;
    ...
    $scope.deleteExampleImg = function(item) {
      ...
      $scope.item.path = $scope.chain.example.path = $scope.item.requirementId +
        $scope.item.functionalityId + $scope.item.exampleId;
        ...
        function(data) {
          ...
          for (var i = $scope.allFunctionalities.length - 1; i >= 0; i--) {
            ...
            for (var j = 0; j < $scope.chain.functionality.links.length; j++) {
              if ($scope.chain.functionality.links[j] == ($scope.allFunctionalities[
                  i].requirementId + $scope.allFunctionalities[i].functionalityId)) {
                $scope.allFunctionalities.splice(i, 1);
              }
            }

            if (($scope.allFunctionalities[i].functionalityId == $scope
                .chain.functionality.functionalityId) && ($scope.allFunctionalities[
                i].requirementId == $scope.chain.functionality.requirementId)) {
              $scope.allFunctionalities.splice(i, 1);
            }
          }
        });
    ...
    $scope.selectLink = function(link) {
      ...
      var reqId = link.substring(0, link.indexOf('F'));
      var funcId = link.substr(link.indexOf('F'));

      for (var i = 0; i < $scope.allRequirements.length; i++) {
        if ($scope.allRequirements[i].requirementId == reqId) {
          chain.requirement = $scope.allRequirements[i];
        }
      }
